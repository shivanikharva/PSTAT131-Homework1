...
### Question 3   HELP
(HELP what it mean by metric????)  ANY MEASURE OF THE MODELS PERFORMANCE accuracy, f1 score - classification
mse and root mse - regression
...
( IS THIS JUST OPINION???)Mechanistic model is easier to understand because assumptions can be made about the model whereas in empirically-driven models, everything must be deduced from the data. 

Mechanistic models tend to be easier to explain and empirically driven can become a lot harder to explain
...
```{r}
hwy_cyl_boxplot <- ggplot(mpg, aes(x=hwy, y=cyl, group=cyl, fill=cyl)) +
  geom_boxplot()
...
Each of the variables has a perfect positive correlation with itself, which makes sense because all of the values are the same.  

Year appears to have no correlation (with cty and hwy) or a very weak positive correlation (with cyl and displ) with all the variables. This indicates that each of the variables do not really vary depending on the year.  
