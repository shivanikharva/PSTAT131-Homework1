...
hwy_cyl_boxplot
```
There is an apparent pattern shown with the boxplots. The highway miles per gallon appear to generally decrease as the cylinder count increases. However, at the 5 cylinder count, the general pattern is disrupted as the highway miles per gallon do not show a general decrease compared to the 4 cylinder count (except at the minimum and first quartile, which both decreased when comparing the 5 cylinder boxplot to the 4 cylinder boxplot).  
...
`hwy` & `cty` have a strong positive correlation (0.96), which makes sense because, if individuals in certain cities are getting more miles per gallon on the highway, those same individuals are getting more miles per gallon in their respective cities.   
